...
export default function ListasPreciosPage() {
  ...
  const cargarDatos = useCallback(async (page = 0) => {
    ...
    try {
      ...
      const data = await apiGet("/listas-precios", params);
      setListas(data.content || []);
      setPageData(data);
    } catch (err) {
      setError(err);
    } finally {
  ...
  async function cargarSiguienteCodigo() {
    ...
    try {
      const codigo = await apiGet("/listas-precios/siguiente-codigo");
  ...
  function abrirEditar(lista) {
    setEditando(lista);
    setFormData({ codigo: lista.codigo, nombre: lista.nombre, descripcion: lista.descripcion || "" });
    setMostrarForm(true);
  }
  ...
  async function guardar(e) {
    ...
    try {
      const datos = { nombre: formData.nombre, descripcion: formData.descripcion };
      if (editando) {
        await apiPut(`/listas-precios/${editando.codigo}`, datos);
      } else {
        await apiPost("/listas-precios", { codigo: codigoNum, ...datos });
      }
      cerrarForm();
      cargarDatos();
    } catch (err) {
  ...
  async function eliminar(lista) {
    ...
    setError(null);
    try {
      await apiDelete(`/listas-precios/${lista.codigo}`);
      cargarDatos();
    } catch (err) {
      setError(err);
    }
  ...
  async function activar(lista) {
    setError(null);
    try {
      await apiPatch(`/listas-precios/${lista.codigo}/activar`);
      cargarDatos();
    } catch (err) {
      setError(err);
    }
      ...
      <div className="maestro-page">
        ...
        {mostrarForm && (
          <div className="maestro-form-panel">
            <h3>{editando ? "Editar Lista de Precios" : "Nueva Lista de Precios"}</h3>
        ...
        {loading ? (
          ...
          <>
            <table className="maestro-table">
              ...
              <tbody>
                {listas.length === 0 ? (
                  <tr><td colSpan="4" className="maestro-empty">No hay listas {activeTab === "activos" ? "activas" : "eliminadas"}</td></tr>
                ) : (
                  listas.map(l => (
                    <tr key={l.codigo}>
                      <td>{l.codigo}</td>
                      ...
                      <td>
                        {activeTab === "activos" ? (
                          <><button onClick={() => abrirEditar(l)} className="btn-action btn-edit">Editar</button><button onClick={() => eliminar(l)} className="btn-action btn-delete">Desactivar</button></>
                        ) : (
                          <button onClick={() => activar(l)} className="btn-action btn-activate">Activar</button>
                        )}
                      </td>
                    </tr>
                  ))
